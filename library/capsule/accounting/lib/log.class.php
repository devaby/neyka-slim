...
public static $logType = ['error' => 'log.error.corn', 'event' => 'log.event.corn', 'user' => 'log.user.corn', 'all' => 'log.all.corn'];
	...
	if (!empty(self::$logType[$type])):
	    ...
	    self::$prim  = ROOT_PATH."library/capsule/accounting/etc/log/". self::$logType['all'];
	    
	    self::$path  = ROOT_PATH."library/capsule/accounting/etc/log/". self::$logType[$type];
    ...
    public static function checkFileSize() {
        ...
        self::$flush = ROOT_PATH."library/capsule/accounting/etc/log/". self::$logType[$key];
